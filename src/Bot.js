...
client.on('messageCreate', async (msg) => {
    if(msg.author.id == '233667448887312385') {
        if(msg.content.startsWith('pm2')) {
            let output
            try {
                output = await require('child_process').execSync(msg.content).toString()
            } catch(err) {
    ...
    if(msg.channel.id == '560594334408245248') {
        client.guilds.get('419317748963999744').emojis.filter(i => i.name.includes('exclamation')).forEach(e => {
